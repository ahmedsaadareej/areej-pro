...
router.get('/labels', (req, res) => {
  try {
    ...
    const rows = db.prepare(`
      SELECT l.*,
        (SELECT COUNT(*) FROM inbox_conversation_labels cl
...
router.post('/labels', (req, res) => {
  try {
    ...
    const result = db.prepare('INSERT INTO inbox_labels (name, color) VALUES (?, ?)').run(name, color);
    res.json({ ok: true, id: result.lastInsertRowid });
  } catch (e) {
    if (e.message.includes('UNIQUE')) {
      return res.status(409).json({ ok: false, error: 'label exists' });
    }
...
// DELETE /api/inbox/labels/:labelId
...
router.delete('/conversations/:id/labels/:labelId', (req, res) => {
  try {
    ...
    db.prepare('DELETE FROM inbox_conversation_labels WHERE conversation_id = ? AND label_id = ?')
      .run(convId, labelId);
